    ...
    public function translate($filename = '')
    {
        ...
        foreach ($bodyNode->childNodes as $child) {
            ...
            if ($this->table2text && $child->tagName == 'w:tbl') {
                ...
                // $this->textOutput .= $this->table($child) . $this->separator();
                array_push($this->textOutput, $this->table($child));
            } else {
                //this node is a paragraph
                // $this->textOutput .= $this->printWP($child) . ($this->paragraph2text ? $this->separator() : '');
                array_push($this->textOutput,  $this->printWP($child));
            }
        }
        if (!empty($filename)) {
            $this->writeFile($filename, $this->textOutput);
        } else {
